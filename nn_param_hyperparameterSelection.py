...
def train_NN(num_nodes, activation, X_train, X_val, y_train, y_val, learning_rate, sigma_sq, batch_size):
	...
	if activation == 'RELU':
		...
		h_1 = tf.nn.sigmoid(tf.matmul(w_1, X_in))
		h_2 = tf.nn.sigmoid(tf.matmul(w_2, h_1))
		y_pred = tf.matmul(w_3, h_2)
	else:
...
def compare_baseline(activation, y_test, X_test, w):
	...
	if activation == 'RELU':
		...
		h_1_test = tf.nn.sigmoid(tf.matmul(w_1, X_in_test))
		h_2_test = tf.nn.sigmoid(tf.matmul(w_2, h_1))
		y_pred = tf.matmul(w_3, h_2_test)
	else:
		raise ValueError('model not defined for activation given')
...
def config_tf():
	...
	config = tf.ConfigProto(intra_op_parallelism_threads=NUM_PARALLEL_EXEC_UNITS, inter_op_parallelism_threads=NUM_PARALLEL_EXEC_UNITS, allow_soft_placement=True, device_count = {'CPU': NUM_PARALLEL_EXEC_UNITS })
...
if __name__ == '__main__':
	...
	#for num_nodes in [5,10,25,50]:
	#for activation in ['sigmoid']:#['RELU','tanh','sigmoid']:
	for sigma_sq in [50.,25.,5.,1.,1./5.,1./25.,1./50.]:
	#for batch_size in [8,16,32,64,128,256]:
	
		os.chdir(main_dir)
		#f = open('param_crossVal_numNodes'+str(num_nodes)+'.txt', 'a')
		#f = open('param_crossVal_activation'+str(activation)+'.txt', 'a')
		f = open('param_crossVal_sigmaSq'+str(sigma_sq)+'.txt', 'a')
		#f = open('param_crossVal_batch'+str(batch_size)+'.txt', 'a')
